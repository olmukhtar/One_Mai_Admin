...
export default function Affiliates() {
  ...
  // Check permissions - Same as normal users (getAllUsers applies to both)
  ...
  const fetchUsers = (pageNum: number, search?: string) => {
    ...
    apiFetch(url.toString(), {
      ...
      .then(async (res) => {
        if (!res.ok) {
          let msg = `Failed to load affiliates: ${res.status}`;
          try {
      ...
      .catch((e: any) => {
        if (e.name !== "AbortError") setErr(e?.message || "Failed to load affiliates");
      })
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              {canViewDetails ? (
                <Link
                  to={`/users/${row._id}`}
                  state={{ from: location, fromPage: "affiliates" }}
  ...
  const actionItems = useMemo(() => {
    ...
    if (canViewDetails) {
      actions.push({
        label: "View Details",
        onClick: (row: User) => navigate(`/users/${row._id}`, { state: { from: location, fromPage: "affiliates" } }),
      });
    }
      ...
      <div className="space-y-6">
        ...
        {(hasLimitedView || hasReadOnly) && (
          <div className="text-sm text-blue-600 bg-blue-50 p-2 rounded border border-blue-100">
            ...
            {hasReadOnly && "Read-only mode - You can view affiliate information but cannot make changes."}
          </div>
        )}
